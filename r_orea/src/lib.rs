use core::panic;
use std::io::{BufReader,BufWriter, Seek, SeekFrom, Read, BufRead,Result, Write};
use std::path::Path;
use std::fs::File;
use pyo3::prelude::*;
...
impl LogManager
{
    ...
    #[new]
    pub fn new(obj: PyRawObject, filepath : PyObject)-> PyResult<()>
    {

        let reader = YAMLReader::new(&mut filepath.to_string());

        let path = Path::new(&mut filepath.to_string());
        let bin_file = match File::open(path)
        {
            Err(why) => panic!("couldn't open {} : {}", path.display(), why),
            Ok(file) => file,
    ...
    pub fn peek(&mut self) -> PyResult<(String,u8)>
    {
        ...
        Ok((String::from_utf8(byte_arr[6..32].to_vec()).unwrap(), byte_arr[40]))
    }
    ...
    pub fn deserialize(&mut self)-> PyResult<Option<Yaml>>
    {
        
        if self.current_doc_extend==(0,0) { return Ok(None); }        
        let docs = YamlLoader::load_from_str(&self.document_string()).unwrap();
        let dict = docs[0].clone();
        Ok(Some(dict))
    }
    ...
    pub fn slice(&mut self, up :u32, down : u32) -> Vec<Yaml>
    {
        ...
        let _x =self.reader.seek(SeekFrom::Start(start_exten.0));
    ...
    pub fn new_entry (&mut self, a:PyObject) -> PyResult<()>
{   
    let str = a.to_string();
    let yams = YamlLoader::load_from_str(&str).unwrap();
    let mut yamstring = String::new();
    let mut emitter = YamlEmitter::new(&mut str);
    for yam in yams
    {
        let _x = emitter.dump(&yam);
    }

    self.writer.seek(SeekFrom::End(0));
    self.writer.write_all(yamstring.as_bytes());
...
#[pymodule]
fn orea(_py:Python, m :&PyModule) -> PyResult<()>
{
